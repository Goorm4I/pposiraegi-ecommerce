...
#         ./scripts/seed.sh http://pposiraegi-alb-xxx.ap-southeast-2.elb.amazonaws.com
# =============================================================
set -e
...
# 날짜 계산 (python3 사용 - macOS/Linux 공통)
NOW=$(python3 -c "from datetime import datetime, timezone; print(datetime.now().strftime('%Y-%m-%dT%H:%M:%S'))")
# ACTIVE용: 5초 후 시작 → 스케줄러(매 1분)가 PENDING→ACTIVE 자동 전환
SOON=$(python3 -c "from datetime import datetime, timedelta; print((datetime.now() + timedelta(seconds=5)).strftime('%Y-%m-%dT%H:%M:%S'))")
...
# ── 1. 강아지 사료 (ACTIVE: 1시간 전 시작, 1시간 후 종료)
echo "[1/5] 강아지 유기농 사료..."
...
# ── 2. 캣타워 (ACTIVE: 30분 전 시작, 2시간 후 종료)
echo "[2/5] 5단 캣타워..."
call_api POST /api/v1/admin/time-deals/with-product "$(cat <<EOF
{
  "product": {
...
# ── 3. 강아지 간식 (ACTIVE: 1시간 전 시작, 3시간 후 종료)
echo "[3/5] 강아지 수제 간식..."
call_api POST /api/v1/admin/time-deals/with-product "$(cat <<EOF
{
  "product": {
